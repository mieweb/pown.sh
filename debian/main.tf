...
resource "aws_instance" "debian" {
  ...
  connection {
    type        = "ssh"
    user        = "admin"              # Default user for Red Hat Linux
  ...
  provisioner "file" {
    ...
    destination = "/home/admin/pown.sh"  # Destination path on the EC2 instance
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo bash /home/admin/pown.sh"    # Run the pown.sh script
    ]
  }
